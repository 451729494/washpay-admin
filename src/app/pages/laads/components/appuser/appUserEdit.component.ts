...
@Component({
  selector: 'la-app-user-edit',
  templateUrl: './appUserEdit.html'
})
export class AppUserEdit implements OnInit{
  ...
  public toBack():any {
      ...
      this.router.navigate(['/pages/laads/appuser'], {queryParams: {paramId: ''}});
              ...
              if(res.data.roles){
                for(let i=0;i<res.data.roles.length;i++){
                  for(let m=0;m<this.roleList.length;m++){
                    if(res.data.roles[i].roleId == this.roleList[m].id){
  ...
  public onSubmit(values:Object){
    ...
    if(this.searchForm.valid){
      ...
      let requestParam = {'username':values['username'], 'email':values['email'],'mobile':values['mobile'], 'roles':selRoles,'dispatchedCorpId':parseInt(this.dispatchedCorpId)};
