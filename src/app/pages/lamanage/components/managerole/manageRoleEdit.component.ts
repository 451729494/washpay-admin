...
@Component({
  selector: 'la-corp-manage-role-edit',
  templateUrl: './manageRoleEdit.html'
})
export class ManageRoleEdit implements OnInit{
  ...
  public constructor(fb:FormBuilder,private acRoute:ActivatedRoute,private router: Router,private authService : AuthService,private corpManageRoleService:CorpManageRoleService) {
  ...
  public onSubmit(values:Object){
    ...
    if(this.searchForm.valid){
      ...
      let requestParam = {'id': this.curId,
        'corpId':this.authService.getCorpId(),
       'name': values['name'],
      'nameCn': values['nameCn']};

      this.corpManageRoleService.save(JSON.stringify(requestParam))
        .subscribe(res =>{
          if(res.successed === '00'){
            this.toBack();
          }else {
