...
@Component({
  selector: 'la-manage-cost-invoice',
  templateUrl:'./managecostinvoice.html'
})
export class ManageCostInvoice implements OnInit {
  ...
  public toDelete(curId) {
      ...
      this.manageCostInvoiceService.delete(requestParam)
        .subscribe(res => {
          if (res.successed === '00') {
            this.loadData();
          } else {
  ...
  public toAdd() {
    ...
    this.router.navigate(['/pages/lafinance/costinvoiceedit'], {
      queryParams: {
        paramId: ''
      }
    });
  ...
  public toApply(curId) {
    ...
    this.manageCostInvoiceService.toApplyApprove(JSON.stringify(params))
      .subscribe(res => {
        if (res.successed === '00') {
          this.loadData();
        }
