...
@Component({
  selector: 'la-borrow-invoice-apply',
  templateUrl:'./borrowinvoiceApply.html'
})
export class UserAccountBorrowApply implements OnInit {
  ...
  public toApproved(curId) {
    ...
    this.userAccountBorrowService.toApprove(JSON.stringify(params))
      .subscribe(res => {
        if (res.successed === '00') {
          this.loadData();
        }
