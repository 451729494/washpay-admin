...
@Component({
  selector: 'la-employee-meal',
  templateUrl:'./employeeMeal.html'
})
export class EmployeeMeal implements OnInit {
  ...
  public constructor(fb:FormBuilder, private router:Router,private route:ActivatedRoute,  private userMealRecordService:UserMealRecordService,private modalService: NgbModal, private authService:AuthService) {
  ...
  public loadData() {
    ...
    this.userMealRecordService.pageQuery(requestParam)
      .subscribe(res => {
        if (res.successed === '00') {
          this.rows = res.data;
 ...
 setPage(event){
  ...
  toImport(){
    ...
    customerModal.componentInstance.modalHeader = '员工饭卡记录导入';
    customerModal.componentInstance.uploadFileUrl = '/open/common/uploadMeal';
