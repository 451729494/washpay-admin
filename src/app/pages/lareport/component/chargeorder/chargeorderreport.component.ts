...
@Component({
  selector: 'la-chargeorderreport-query',
  templateUrl:'./chargeorderreport.html'
})
export class ChargeorderreportQuery implements OnInit {
  ...
  public constructor(fb:FormBuilder, private router:Router,private route:ActivatedRoute, private chargeorderrepotService:ChargeorderrepotService,private _dateParser:NgbDateParserFormatter) {
  ...
  public loadData() {
    ...
    this.chargeorderrepotService.pageQuery(requestParam)
      .subscribe(res => {
        if (res.successed === '00') {
          this.rows = res.data;
  ...
  public toDelete(curId) {
    ...
    this.chargeorderrepotService.delete(requestParam)
      .subscribe(res => {
        if (res.successed === '00') {
          this.loadData();
        } else {
